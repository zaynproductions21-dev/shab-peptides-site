...
// Hoisted: static lists never need to be recreated on every render.
const NAV_ITEMS = [
  { label: "Home", href: "/" },
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 ${s.bg} border-b`}>
      <div className="mx-auto max-w-[1200px] px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-3 xl:gap-5 ml-6">
            {NAV_ITEMS.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className={`text-[13px] xl:text-sm font-medium whitespace-nowrap ${s.link} ${isSerif ? "tracking-wide" : ""}`}
              >
                {item.label}
              </Link>
            ))}
            ...
            <div className="hidden xl:flex items-center gap-1.5">
              {SOCIAL_LINKS.map((social) => (
                <a
                  ...
                  className={`inline-flex h-8 w-8 items-center justify-center rounded-full ${s.link}`}
                >
                  {social.icon}
                </a>
              ))}
          ...
          <div className="flex items-center justify-center gap-4 pt-2">
            {SOCIAL_LINKS.map((social) => (
              <a
                ...
                rel="noopener noreferrer"
                aria-label={social.label}
                className={`inline-flex h-9 w-9 items-center justify-center rounded-full ${s.link}`}
                onClick={() => setMobileMenuOpen(false)}
              >
                {social.icon}
              </a>
